...
public class Player2 extends Entity {
    ...
    public void update(Input input) {
        ...
        if (input.getKey(KeyEvent.VK_W) &&
                !Collusion.checkCollusion(newX2, newY2 - speed, SPRITE_SCALE * scale, SPRITE_SCALE * scale, Player.newX1, Player.newY1, SPRITE_SCALE * scale, SPRITE_SCALE * scale) &&
                !Level.bricks.stream().anyMatch(b -> Collusion.checkCollusion( newX2 , newY2 - speed ,SPRITE_SCALE * scale , SPRITE_SCALE * scale ,(float) b.getX() ,(float) b.getY() ,Level.TILE_IN_GAME ,Level.TILE_IN_GAME ))) {
            newY2 -= speed ;
            heading = Heading.NORTH;
        } else if (input.getKey(KeyEvent.VK_D) &&
                !Collusion.checkCollusion(newX2 + speed, newY2, SPRITE_SCALE * scale, SPRITE_SCALE * scale, Player.newX1, Player.newY1, SPRITE_SCALE * scale, SPRITE_SCALE * scale) &&
                !Level.bricks.stream().anyMatch(b -> Collusion.checkCollusion( newX2 + speed , newY2 ,SPRITE_SCALE * scale , SPRITE_SCALE * scale ,(float) b.getX() ,(float) b.getY() ,Level.TILE_IN_GAME ,Level.TILE_IN_GAME))) {
            newX2 += speed ;
        ...
        if (firing) {   //стрельба
            ...
            if (elapsed > firingDelay) {
                if (heading == Heading.NORTH) Game.bullets.add(new Bullet(270, newX2 + (SPRITE_SCALE * scale)/2 - 1, newY2 ));
